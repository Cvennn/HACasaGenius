"""
Number platform for CASA Genius.

Exposes temperature setpoint as a number entity.
SCB 4.1 register 4x5101:
  Range:  130-250 (raw Modbus integer)
  Scale:  ÷10 for display  → 13.0-25.0 °C
  Step:   0.5 °C  (raw step = 5)
  Write:  °C value * 10 → integer sent to Modbus
"""

from __future__ import annotations

from typing import TYPE_CHECKING

from homeassistant.components.number import NumberEntity, NumberMode
from homeassistant.helpers.device_registry import DeviceInfo
from homeassistant.helpers.update_coordinator import CoordinatorEntity

from .const import DOMAIN

if TYPE_CHECKING:
    from homeassistant.config_entries import ConfigEntry
    from homeassistant.core import HomeAssistant
    from homeassistant.helpers.entity_platform import AddEntitiesCallback

    from custom_components.swegon_genius.coordinator import SwegonCoordinator

NUMBER_DEFS = [
    {
        "key": "temp_setpoint",
        "translation_key": "temp_setpoint",
        "address": 5100,
        "min": 13.0,
        "max": 25.0,
        "step": 0.5,
        "unit": "C",
        "scale": 0.1,
    },
    {
        "key": "temp_setpoint_summer",
        "translation_key": "temp_setpoint_summer",
        "address": 5167,
        "min": 13.0,
        "max": 25.0,
        "step": 0.5,
        "unit": "C",
        "scale": 0.1,
    },
    {
        "key": "temp_setpoint_away",
        "translation_key": "temp_setpoint_away",
        "address": 5170,
        "min": 13.0,
        "max": 25.0,
        "step": 0.5,
        "unit": "C",
        "scale": 0.1,
    },
    {
        "key": "room_temp_setpoint",
        "translation_key": "room_temp_setpoint",
        "address": 5186,
        "min": 16.0,
        "max": 26.0,
        "step": 0.5,
        "unit": "C",
        "scale": 0.1,
    },
    {
        "key": "co2_boost_limit",
        "translation_key": "co2_boost_limit",
        "address": 5115,
        "min": 0,
        "max": 2000,
        "step": 50,
        "unit": "ppm",
        "scale": 1.0,
    },
    {
        "key": "co2_home_limit",
        "translation_key": "co2_home_limit",
        "address": 5113,
        "min": 0,
        "max": 2000,
        "step": 50,
        "unit": "ppm",
        "scale": 1.0,
    },
    {
        "key": "co2_away_limit",
        "translation_key": "co2_away_limit",
        "address": 5114,
        "min": 0,
        "max": 2000,
        "step": 50,
        "unit": "ppm",
        "scale": 1.0,
    },
    {
        "key": "home_plus_level",
        "translation_key": "home_plus_level",
        "address": 5107,
        "min": 10,
        "max": 90,
        "step": 5,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "away_supply_fan",
        "translation_key": "away_supply_fan",
        "address": 5301,
        "min": 20,
        "max": 100,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "away_exhaust_fan",
        "translation_key": "away_exhaust_fan",
        "address": 5302,
        "min": 20,
        "max": 100,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "home_supply_fan",
        "translation_key": "home_supply_fan",
        "address": 5303,
        "min": 20,
        "max": 100,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "home_exhaust_fan",
        "translation_key": "home_exhaust_fan",
        "address": 5304,
        "min": 20,
        "max": 100,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "boost_supply_fan",
        "translation_key": "boost_supply_fan",
        "address": 5305,
        "min": 20,
        "max": 100,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "boost_exhaust_fan",
        "translation_key": "boost_exhaust_fan",
        "address": 5306,
        "min": 20,
        "max": 100,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "cooking_supply_fan",
        "translation_key": "cooking_supply_fan",
        "address": 5183,
        "min": 20,
        "max": 100,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "cooking_exhaust_fan",
        "translation_key": "cooking_exhaust_fan",
        "address": 5184,
        "min": 20,
        "max": 100,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    # Sorptioroottorin kosteudenhallinta (vain Sorption-yksiköissä)
    {
        "key": "indoor_humidity_setpoint",
        "translation_key": "indoor_humidity_setpoint",
        "address": 5224,
        "min": 30,
        "max": 65,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "indoor_humidity_limit_cold",
        "translation_key": "indoor_humidity_limit_cold",
        "address": 5225,
        "min": 10,
        "max": 50,
        "step": 1,
        "unit": "%",
        "scale": 1.0,
    },
    {
        "key": "defrost_min_supply_temp",
        "translation_key": "defrost_min_supply_temp",
        "address": 5197,
        "min": 10.0,
        "max": 17.0,
        "step": 0.5,
        "unit": "C",
        "scale": 0.1,
    },
    # Kanavapaineensäätö (Genius) — kiinteät asetusarvot
    {
        "key": "supply_duct_pressure_setpoint",
        "translation_key": "supply_duct_pressure_setpoint",
        "address": 5320,
        "min": 0,
        "max": 100.0,
        "step": 0.1,
        "unit": "Pa",
        "scale": 0.1,
    },
    {
        "key": "extract_duct_pressure_setpoint",
        "translation_key": "extract_duct_pressure_setpoint",
        "address": 5321,
        "min": 0,
        "max": 100.0,
        "step": 0.1,
        "unit": "Pa",
        "scale": 0.1,
    },
    # Kanavapaineensäätö (Genius) — BMS-säädettävät asetusarvot
    {
        "key": "supply_duct_pressure_setpoint_adjustable",
        "translation_key": "supply_duct_pressure_setpoint_adjustable",
        "address": 5322,
        "min": 0,
        "max": 100.0,
        "step": 0.1,
        "unit": "Pa",
        "scale": 0.1,
    },
    {
        "key": "extract_duct_pressure_setpoint_adjustable",
        "translation_key": "extract_duct_pressure_setpoint_adjustable",
        "address": 5323,
        "min": 0,
        "max": 100.0,
        "step": 0.1,
        "unit": "Pa",
        "scale": 0.1,
    },
    # Ruoanlaittotilan paineasetukset
    {
        "key": "cooking_supply_pressure",
        "translation_key": "cooking_supply_pressure",
        "address": 5324,
        "min": 0,
        "max": 1000.0,
        "step": 0.1,
        "unit": "Pa",
        "scale": 0.1,
    },
    {
        "key": "cooking_exhaust_pressure",
        "translation_key": "cooking_exhaust_pressure",
        "address": 5325,
        "min": 0,
        "max": 1000.0,
        "step": 0.1,
        "unit": "Pa",
        "scale": 0.1,
    },
]


async def async_setup_entry(
    _hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback
) -> None:
    """Set up Swegon Genius number entities from a config entry."""
    coordinator = entry.runtime_data
    async_add_entities([SwegonNumber(coordinator, entry, d) for d in NUMBER_DEFS])


class SwegonNumber(CoordinatorEntity, NumberEntity):
    """Representation of a Swegon Genius number entity."""

    def __init__(
        self, coordinator: SwegonCoordinator, entry: ConfigEntry, number_def: dict
    ) -> None:
        """Initialize the Swegon Genius number entity."""
        super().__init__(coordinator)
        self._def = number_def
        self._attr_unique_id = f"{entry.entry_id}_{number_def['key']}"
        self._attr_has_entity_name = True
        self._attr_translation_key = self._def["translation_key"]
        self._attr_native_min_value = number_def["min"]
        self._attr_native_max_value = number_def["max"]
        self._attr_native_step = number_def["step"]
        self._attr_native_unit_of_measurement = number_def["unit"]
        self._attr_mode = NumberMode.BOX
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, entry.entry_id)},
            name=entry.title,
            manufacturer="Swegon",
            model=coordinator.device_info_data.get("model", "CASA Genius"),
            sw_version=coordinator.device_info_data.get("firmware"),
        )

    @property
    def native_value(self):  # noqa: ANN201
        """Return the current value of the number entity."""
        return self.coordinator.data.get(self._def["key"])

    async def async_set_native_value(self, value: float) -> None:
        """Set the value of the number entity."""
        scale = self._def.get("scale", 1.0)
        raw = round(value / scale) if scale != 1.0 else int(value)
        await self.coordinator.client.write_register(self._def["address"], raw)
        await self.coordinator.async_request_refresh()
